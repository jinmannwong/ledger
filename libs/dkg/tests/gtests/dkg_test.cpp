...
TEST(dkg, trial)
{
  ...
  for (auto ii = 0; ii < cabinet_size; ++ii) {
  ...
  for (auto ii = 0; ii < cabinet_size; ii++) {
      for (auto jj = ii + 1; jj < cabinet_size; jj++) {
          if (jj < 10) {
              committee[ii]->muddle.AddPeer(fetch::network::Uri{"tcp://127.0.0.1:800" + std::to_string(jj)});
          } else {
              committee[ii]->muddle.AddPeer(fetch::network::Uri{"tcp://127.0.0.1:80" + std::to_string(jj)});
          }
      }
  }
  ...
  auto kk = 0;
    while (kk != cabinet_size)
    {
        ...
        for (auto mm = kk; mm < cabinet_size; ++mm) {
  ...
  // Start at RBC for each muddle
  {
    ...
    while (ii != cabinet_size) {
        ...
        for (auto jj = ii; jj < cabinet_size; ++jj) {
            if (!committee[jj]->dkg_service.dkg_completed) {
                break;
            } else {
                ++ii;
            }
    ...
    for (ii = 1; ii < cabinet_size; ++ii) {
        EXPECT_EQ(committee[0]->dkg_service.GroupPublicKey() == committee[ii]->dkg_service.GroupPublicKey(), true);
    }
  }
